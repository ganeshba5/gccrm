 ...
 */
async function getGmailClient(gmailUserEmail) {
    var _a;
    try {
        ...
        const userEmail = gmailUserEmail || process.env.GMAIL_USER_EMAIL ||
            ((_a = functions.config().gmail) === null || _a === void 0 ? void 0 : _a.user_email) ||
        ...
        if (serviceAccountKeyEnv && serviceAccountKeyEnv.trim().length > 0) {
            try {
                ...
                const jwtClient = new googleapis_1.google.auth.JWT(serviceAccount.client_email, undefined, serviceAccount.private_key, ['https://www.googleapis.com/auth/gmail.readonly'], userEmail);
        ...
        if (gmailClientId && gmailClientSecret && gmailRefreshToken) {
            const oauth2Client = new googleapis_1.google.auth.OAuth2(gmailClientId, gmailClientSecret, 'urn:ietf:wg:oauth:2.0:oob' // Redirect URI for installed apps
            );
...
function extractEmailBody(payload) {
    ...
    if ((_a = payload.body) === null || _a === void 0 ? void 0 : _a.data) {
        ...
        const inferredMimeType = ((_c = (_b = payload.parts) === null || _b === void 0 ? void 0 : _b[0]) === null || _c === void 0 ? void 0 : _c.mimeType) || 'text/plain';
 ...
 */
function extractAttachments(messageId, payload) {
    const attachments = [];
    function extractFromPart(part, index) {
        var _a;
        if (((_a = part.body) === null || _a === void 0 ? void 0 : _a.attachmentId) && part.filename) {
            attachments.push({
                id: `${messageId}-${index}`,
        ...
        if (part.parts) {
            part.parts.forEach((p, i) => extractFromPart(p, `${index}-${i}`));
        }
    }
    if (payload.parts) {
        payload.parts.forEach((part, index) => extractFromPart(part, index));
    }
    return attachments;
}
/**
 ...
 */
async function storeEmail(gmailMessage, body, attachments) {
    try {
 ...
 */
async function fetchNewEmails(gmailUserEmail) {
    var _a;
    try {
        const userEmail = gmailUserEmail || process.env.GMAIL_USER_EMAIL ||
            ((_a = functions.config().gmail) === null || _a === void 0 ? void 0 : _a.user_email) ||
 ...
 */
exports.fetchEmails = functions.https.onRequest(async (req, res) => {
    var _a;
    // Set CORS headers
